...
UdpCS::UdpCS(QWidget *parent, int localBindPort)
    : QDialog(parent)
    {	
	m_udpRecvSocket.bind(localBindPort);	
	
	//If have accepted data, process them
	connect(&m_udpRecvSocket, SIGNAL(readyRead()), this, SLOT(receivedData()));
    }
...
void UdpCS::sendDataToServer(QString sendData, int serverPort)
	...
	qint64 returnValue = m_udpSendSocket.writeDatagram(barrSendData, 
...
QString UdpCS::receivedData()
	...
	do {
	    barrDatagram.resize(m_udpRecvSocket.pendingDatagramSize());
	    m_udpRecvSocket.readDatagram(barrDatagram.data(), barrDatagram.size());
	} while (m_udpRecvSocket.hasPendingDatagrams());
...
void UdpCS::disConnectAll()
	{
	disconnect(&m_udpRecvSocket, SIGNAL(readyRead()), this, SLOT(processData()));			
	}
